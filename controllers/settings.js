...
/* =========================================================
   CAFE SETTINGS — per shop
   ========================================================= */

exports.getSettings = async (req, res) => {
  try {
    const rows = await DB.PostgresAny(
      `SELECT key, value FROM cafe_settings WHERE shop_id = $1`,
      [req.shop_id]
    );
    const settings = {};
    ...
    for (const key of allowed) {
      if (req.body[key] !== undefined) {
        ...
        await DB.PostgresAny(
          `INSERT INTO cafe_settings (key, value, shop_id, updated_at)
           VALUES ($1, $2, $3, NOW())
           ON CONFLICT (shop_id, key) DO UPDATE SET value = $2, updated_at = NOW()`,
          [key, val, shopId]
        );
        updates.push(key);
      }
